...
export default {
    ...
    async execute({ res, end, url, user }) {
        ...
        if (!user.staff) for (let key of keys) if (!config.roster.signupEdits.includes(key)) return end(403, { error: 'Staff Only', details: `Only staff can edit column "${key}"` });
        ...
        if (!(user.staff || signup.player_id == user.id)) return end(403, { error: 'Staff Only', details: 'Only staff can edit other users\' signups' });
